...
/**
 * Retry policy with exponential backoff and jitter for HubSpot API rate limits.
...
public class RetryPolicy {
     ...
     */
    public <T> T execute(Callable<T> action) throws HubSpotException {
        int attempt = 0;
        while (true) {
            try {
                return action.call();
                ...
                throw new HubSpotException("Unexpected error during API call", e);
            }
        }
    }
